...
imf_memory_ostream::imf_memory_ostream( std::size_t mem_size) : Imf::OStream(""), buffer_( 0)
{
    ...
    try
    {
        buffer_ = reinterpret_cast<char*>( app().memory_manager().image_allocator().allocate( mem_size));
    }
    catch( std::bad_alloc&)
    {
        errno = ENOMEM;
...
{
    if( buffer_)
        app().memory_manager().image_allocator().deallocate( reinterpret_cast<unsigned char *>( buffer_), mem_size_);
}
