    ...
    void operator()( const tbb::blocked_range<int>& range) const
    {
		for( int j = range.begin(); j < range.end(); ++j)
		{
			...
			for( int i = 0, e = src_.width(); i < e; ++i)
			{
					...
					*dst_it = image::pixel_t( std::max( xr, 0.0f), std::max( xg, 0.0f), std::max( xb, 0.0f),
...
{
    RAMEN_ASSERT( src.dimensions() == dst.dimensions() && "Apply color matrix, src and dst must have the same size");
